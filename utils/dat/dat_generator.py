# -*- coding: utf-8 -*-
"""Chinese.dat and CHARACTERS.txt generator."""
...
class VCDatGenerator:
    def __init__(self) -> None:
        self.gxt_data: Dict[str, Set[str]] = {}
        self.wide_char_collection = {ord(ch) for ch in MOST_USED_CHARACTERS}

    def load_text(self, path: str | Path, *, clear: bool = True) -> None:
        table_format = re.compile(r"^\[([0-9A-Z_]{1,7})\]$")
        entry_format_eq = re.compile(r"^([0-9A-Z_]{1,7})=(.+)$")
        entry_format_ws = re.compile(r"^([0-9A-Z_]{1,7})[\t ]+(.+)$")
        ...
        with open(path, "r", encoding="utf-8-sig", newline="") as input_file:
            for raw_line in input_file:
                ...
                if table_match:
                    ...
                    if current_table not in self.gxt_data:
                        self.gxt_data[current_table] = set() # type: ignore
                    continue

                entry_match = entry_format_eq.match(line) or entry_format_ws.match(line)
                if entry_match:
                    if current_table is None:
                        current_table = default_table
                        if current_table not in self.gxt_data:
                            self.gxt_data[current_table] = set()
                    ...
                    if value.count("~") % 2 != 0:
                        print(
                            f"Invalid token format at key: {key} in {Path(path).as_posix()}. Skipped."
                        )
                        continue

                    if key in self.gxt_data[current_table]:
                        raise ValueError(
                            f"Repeated entry {key} in table {current_table} from {Path(path).as_posix()}."
                        )

                    self.gxt_data[current_table].add(key)

                    for ch in value:
                        if ord(ch) >= 0x80:
                            self.wide_char_collection.add(ord(ch))

                    continue

                raise ValueError(f"Invalid line in {Path(path).as_posix()}: {line}")

    def load_texts(self, paths: list[Path]) -> None:
        self.gxt_data.clear()
        for idx, path in enumerate(paths):
            self.load_text(path, clear=False)

    def generate(self, characters_path: str | Path, table_path: str | Path) -> None:
        characters_path = Path(characters_path)
        table_path = Path(table_path)

        table_bytes = bytearray([63]) * (0x10000 * 2)

        row = 0
        column = 0

        with characters_path.open("wb") as characters_file:
            characters_file.write(b"\xFF\xFE")

            for codepoint in sorted(self.wide_char_collection):
                characters_file.write(chr(codepoint).encode("utf-16le"))

                index = codepoint * 2
                if index + 1 < len(table_bytes):
                    table_bytes[index] = row & 0xFF
                    table_bytes[index + 1] = column & 0xFF
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Chinese.dat/CHARACTERS.txt generator")
    parser.add_argument("input", help="Input .txt file or directory")
    parser.add_argument(
        "--table",
        default="Chinese.dat",
        help="Output Chinese.dat path",
    )
    parser.add_argument(
        "--characters",
        default="CHARACTERS.txt",
        help="Output CHARACTERS.txt path",
    )

    args = parser.parse_args()

    generator = VCDatGenerator()
    input_path = Path(args.input)
    if input_path.is_dir():
        txt_paths = sorted(p for p in input_path.rglob("*.txt") if p.is_file())
        if not txt_paths:
            raise ValueError(f"No .txt files found under {input_path.as_posix()}.")
        generator.load_texts(txt_paths)
    else:
        generator.load_text(input_path)
    generator.generate(args.characters, args.table)
